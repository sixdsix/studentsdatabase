...
def addNew():
    ...
    while not zipCode.isnumeric() or not len(str(zipCode)) == 5:
        print('ERROR: Must be 5 digits')
        zipCode = input('Enter Zip Code: ')

    phoneNumber = input('Enter Phone Number: ')

    major = input('Enter Major: ')
    while major.isnumeric():
        print('ERROR: Must be Alphabetical')
        major = input('Enter Major: ')

    advisor = input('Input Advisor: ')
    while advisor.isnumeric():
        print('ERROR: Must be Alphabetical')
    ...
    while boolean:
        try:
            float(gpa)
            while float(gpa) > 5.0:
                print("ERROR Must be less than 5.0")
                gpa = input('Enter GPA: ')
                boolean = True
            boolean = False
        except ValueError:
...
def deleteStudent():
    ...
    rows = mycursor.fetchall()
    # check to make sure it is inside the size of the database
    while int(ID) > int(rows[0][0]) or not ID.isnumeric():
        print('ERROR: Please enter a valid ID')
        ID = input('What is the student ID of the student you seek to delete?: ')
